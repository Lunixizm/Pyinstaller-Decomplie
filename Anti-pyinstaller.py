...
def get_pyc_file():
    ...
    # Parametreden dosya adını normalleştir ve uzantıyı temizle
    dosya_adi = os.path.basename(sys.argv[1])
    dosya_adi_temiz = os.path.splitext(dosya_adi)[0]

    # Hedef klasör: {parametre}_extracted
    hedef_klasor = os.path.join(betik_dizini, f"{dosya_adi}_extracted")

    # .pyc dosyasını hedef klasörde ara
    pyc_dosya_adi = f"{dosya_adi_temiz}.pyc"
    pyc_dosya_yolu = os.path.join(hedef_klasor, pyc_dosya_adi)

    if not os.path.exists(pyc_dosya_yolu):
        print(f"{pyc_dosya_adi} dosyası {hedef_klasor} içinde bulunamadı.")
        return

    # .pyc dosyasını betiğin bulunduğu dizine kopyala
